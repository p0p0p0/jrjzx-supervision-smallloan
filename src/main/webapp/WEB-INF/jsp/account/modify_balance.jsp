...
<body>
	...
	<div class="container pt100">
		<div class="add-form add-finance-data panel">
			<h3>增加银行账户余额报备</h3>
			<form method="post" action="account/upd_balance">
				...
				<div class="form-group row">
					...
					<div class="col-sm-6">
						<input type="text" class="form-control form_monthtime"
							name="dateMonth" value="${balance.dateMonth }" />
					</div>
				</div>
				<div class="form-group row">
						...
						<select class="form-control" name="accountId">
							<c:forEach items="${accounts }" var="account">
								<option value="${account.id }"
									<c:if test="${account.id==balance.accountId }">selected="selected"</c:if> />${account.account }(${account.bankOption.optionValue })
						</c:forEach>
						</select>
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="input-group col-sm-6" style="margin-left:15px;">
						<input type="text" class="form-control" name="money"
							value="${balance.money }" /> <span class="input-group-addon">元</span>
					</div>
				</div>
