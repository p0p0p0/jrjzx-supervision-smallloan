...
<body>
	...
	<div class="container clearfix pt100">
		<div class="add-form add-finance-data panel clearfix ">
			<h3>添加账户</h3>
			<form class="clearfix" method="post" action="account/upd_account">
				...
				<div class="form-group row">
					...
					<div class="col-sm-6">
						<select class="form-control" name="type">
							<option value="1"
								<c:if test="${account.type==1 }">selected="selected"</c:if>>基本户</option>
							<option value="2"
								<c:if test="${account.type==2 }">selected="selected"</c:if>>一般户</option>
							<option value="3"
								<c:if test="${account.type==3 }">selected="selected"</c:if>>监管专用户</option>
						</select>
					</div>
				</div>
				<div class="form-group row">
						...
						<select class="form-control" name="bank">
							<c:forEach items="${bank.dataOptions }" var="option">
								<option value="${option.optionKey }"
									<c:if test="${option.optionKey eq account.bank  }">selected="selected"</c:if> />${option.optionValue }
                    	</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="form-group row clearfix">
					...
					<div class="col-sm-6">
						<input class="form-control form_datetime" type="text"
							name="openTime" value='<fmt:formatDate value="${account.openTime }" type="both"
										pattern="yyyy-MM-dd" />' />
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-sm-6">
						<select class="form-control" name="accountStatus">
							<c:forEach items="${status.dataOptions}" var="option">
								<option value="${option.optionKey }"
									<c:if test="${option.optionKey eq account.accountStatus  }">selected="selected"</c:if> />${option.optionValue }
                    </c:forEach>
